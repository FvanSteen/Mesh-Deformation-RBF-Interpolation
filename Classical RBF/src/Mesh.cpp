		...
		while (getline(mFile, line)){
			...
			// Check whether line corresponds to ext. boundary
				...
				if(line.rfind("MARKER_ELEMS= ",0)==0){
					...
					getExtBdryData(line, cntExtElems, cntExtNodes,ebTags.size(),bdryElems);
				}
			}
...
void Mesh::getExtBdryData(std::string& line, int& cntExtElems, int& cntExtNodes, int nBdry, int bdryElems){
